...
DB_NAME = 'ServerStats'                                     # Influxdb name
...
def addPointsSqlite( server, files, size, startTime, endTime ):
   if os.path.isfile( SQLITE_DB ):                       # Checking for the SQLITE DB
      ...
      cursor.execute('''INSERT INTO drive_info(server,files,bytes,starttime,endtime,uploaded)
                  VALUES(?,?,?,?,?,?)''', (server, files, size, startTime, endTime,0))
      
      conn.commit()
      conn.close()
      addPointsSqlToInfluxdb()
   else:
